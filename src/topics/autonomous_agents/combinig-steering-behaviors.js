module.exports = function (p) {
	...
	class Vehicle {
		...
		combinedBehavior(vehicles) {
			// console.log(this.separate(vehicles));
	...
	p.draw = () => {
		...
		for(let i = 0; i < vehicles.length; i++) {
			vehicles[i].combinedBehavior(vehicles);
			vehicles[i].edge();
			vehicles[i].run();
		}
		...
		p.fill(0);
		p.noStroke();
		p.text("Everything gets separated and seeked towards mouse", 150, 15);
	}
